...
public class InventoryService : Inventory.InventoryBase
{
    ...
    public override Task<DeductResponse> Deduct(DeductRequest request, ServerCallContext context)
    {
        ...
        if (!request.Items.Any())
        {
            ...
            {
                Success = false,
                Message = "[Inventory] No items found in request"
            });
        }
        ...
        foreach (var item in request.Items)
        {
            ...
            if (dbItem == null)
            {
                ...
                {
                    Success = false,
                    Message = $"[Inventory] Item {item.Id} not found"
                });
            }
            ...
            if (dbItem.Quantity < item.Quantity)
            {
                ...
                {
                    Success = false,
                    Message = $"[Inventory] Not enough stock for item {item.Id}"
                });
            }
        }
        ...
        {
            Success = true,
            Message = "[Inventory] Items deducted successfully"
        });
    }

    public override Task<DeductResponse> Rollback(DeductRequest request, ServerCallContext context)
    {
        _logger.LogInformation("Inventory Rollback started. Items count: {Count}", request.Items.Count);
        ...
        _logger.LogInformation("Validation passed. Applying rollback...");
        ...
        foreach (var item in request.Items)
        {
            ...
            _logger.LogInformation(
                "Rolling back ItemId: {ItemId}. Before: {BeforeQty}, Rolling back: {Quantity}",
                item.Id, dbItem.Quantity, item.Quantity);
        ...
        _logger.LogInformation("Inventory rollback completed successfully");
